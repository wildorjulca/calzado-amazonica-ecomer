...
import { LinkWithChannel } from "./Nav/LinkWithChannel";
...
export const CartNavItem = async ({ channel }: Props) => {
    ...
    return (
        <div
            // onClick={() => closeCartDrawer && closeCartDrawer()}
            className="relative flex items-center hover:cursor-pointer"
        // data-testid="CartNavItem"
        >

            <ShoppingBagIcon
                className="h-6 w-6 shrink-0" aria-hidden="true" />
            {lineCount > 0 ? (
                <div
                    className={clsx(
